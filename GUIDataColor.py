...
class CameraWorker:
    def __init__(self):
        ...
        self.latest = None   # numpy array (color or gray)
        self.fps = 0.0
    ...
    def start(self):
        ...
        try:
            ...
            # ----- COLOR PIXEL FORMAT -----
            # Request BGR8 (8-bit color). If unsupported, this will raise and we'll just leave default.
            try:
                self.cam.PixelFormat.SetValue(PySpin.PixelFormat_BGR8)
            except PySpin.SpinnakerException:
                # If this fails, camera may still be Mono8 or some default.
                pass
        ...
        while self.running:
            try:
                ...
                if img.IsIncomplete():
                    img.Release()
                    continue
                frame = img.GetNDArray()  # could be Mono8 or BGR8
                img.Release()
                ...
                # Draw FPS text (in-place) for display / snapshots
                disp = frame.copy()
                if disp.ndim == 2:
                    # Gray image
                    disp = cv2.putText(
                ...
                with self.lock:
                    ...
                    if self.recording and self.video_path is not None:
                        # If grayscale, convert to BGR; if already color, keep as is
                        if disp.ndim == 2:
                            frame_bgr = cv2.cvtColor(disp, cv2.COLOR_GRAY2BGR)
                        else:
                            frame_bgr = disp
...
class UMPGui:
    ...
    def _update_camera_view(self):
        ...
        if frame is not None:
            # Convert -> Tk image
            if PIL_AVAILABLE:
                # If frame is color (H,W,3), convert BGR -> RGB for Tk
                ...
                # This basic PGM approach only supports grayscale; if color, convert to gray
                if frame.ndim == 3 and frame.shape[2] == 3:
